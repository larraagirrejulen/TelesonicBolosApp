...
import { Alert, Button, Image, ScrollView, StyleSheet, Switch, Text, TouchableOpacity, View } from "react-native";
import auth from '@react-native-firebase/auth';
import TabBackground from "./TabBackground";
import { SafeAreaView } from "react-native-safe-area-context";

export default function Perfil(user:any){
    return(
        <TabBackground>
            <SafeAreaView style={{ flex: 1 }}>
                <ScrollView contentContainerStyle={{paddingVertical: 24}}>
                    <View style={styles.profile}>
                        <Image alt="" source={{uri: user.user.user.photoURL}} style={styles.profileAvatar} />
                        <View>
                            <Text style={styles.profileName}>{user.user.user.displayName}</Text>
                            <Text style={styles.profileAddress}>{user.user.user.email}</Text>
                        </View>
                    </View>
                    <Button title="Cerrar Sesión" onPress={()=>{
                        Alert.alert('Confirma el cierre de sesión:', '', [
                            {text: 'Aceptar', onPress: () => {auth().signOut(); GoogleSignin.signOut()}},
